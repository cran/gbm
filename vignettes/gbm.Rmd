...
commonly associated with boosting. The Cox proportional hazards model,
...
proposed. So the main purpose of this document is to spell out in
...
work of Friedman, Hastie, and Tibshirani (2000) and laid the groundwork
...
\end{equation} 
Parametric regression models assume that
...
\end{equation} 
When we wish to estimate $f(\mathbf{x})$
...
decrease the $N$ dimensional function 
\begin{eqnarray}
(\#eq:Friedman2)
\hspace{0.5in}
...
\end{eqnarray} 
The negative gradient of $J(\mathbf{f})$ indicates the
direction of the locally greatest decrease in $J(\mathbf{f})$. Gradient
descent would then have us modify $\mathbf{f}$ as
\begin{equation}
(\#eq:Friedman3)
\hspace{0.5in}
\hat{\mathbf{f}} \leftarrow \hat{\mathbf{f}} - \rho \nabla J(\mathbf{f})
\end{equation}
where $\rho$ is the size of the step along the
...
Gradient Boosting algorithm shown in Algorithm 1. At each iteration the algorithm
...
$\Psi(y_i,f(\mathbf{x}_i)) = (y_i-f(\mathbf{x}_i))^2$, this
...
Initialize \(\hat f(\mathbf{x})\) to be a constant,
\(\hat f(\mathbf{x}) = \arg \min_{\rho} \sum_{i=1}^N \Psi(y_i,\rho)\).  
For \(t\) in \(1,\ldots,T\) do

1.  Compute the negative gradient as the working response
    \[
    z_i = -\left.\frac{\partial}{\partial f(\mathbf{x}_i)}
    \Psi(y_i,f(\mathbf{x}_i))\right|_{f(\mathbf{x}_i)=\hat f(\mathbf{x}_i)}
    \]

2.  Fit a regression model, \(g(\mathbf{x})\), predicting \(z_i\) from
    the covariates \(\mathbf{x}_i\).
...
suggested in Algorithm 1. For example, Friedman (2001) substituted
...
greatly improve predictive performance while simultaneously reducing
...
\end{equation} 
So far we have taken advantage of this form only by
...
when $\lambda$ is as small as possible, with decreasing
...
is, if when $\lambda=1.0$ the optimal $T$ is 100 iterations, this does
...
\end{equation} 
This applies to boosted trees. Regression stumps (one
...
multivariate function well, generalized additive models, the naive Bayes
...
\end{equation} 
where $I_t^2$ is the empirical improvement by splitting
...
**Algorithm 2. Boosting as implemented in `gbm()`**

Select

- a loss function (`distribution`)
- the number of iterations, \(T\) (`n.trees`)
- the maximum interaction depth (`interaction.depth`)
...
3.  Fit a regression tree, \(g(\mathbf{x}) = E(z|\mathbf{x})\). This
    tree is fit using only those randomly selected observations. Let
    \(K\) be the number of terminal nodes in the fitted tree.

4.  Compute the optimal terminal node predictions,
    \(\rho_1,\ldots,\rho_K\), as
    \[
    \rho_k = \arg \min_{\rho}
    \sum_{\mathbf{x}_i \in S_k}
    \Psi(y_i,\hat f(\mathbf{x}_i)+\rho)
    \]
    where \(S_k\) is the set of \(\mathbf{x}\)s that define terminal
    node \(k\). Again this step uses only the randomly selected
    observations.
...
5.  Update \(\hat f(\mathbf{x})\) as
    ...
    \]
    where \(k(\mathbf{x})\) indicates the index of the terminal node into
    which an observation with features \(\mathbf{x}\) would fall.
...
```{r shrinkViters, echo=FALSE, out.width="100%", fig.cap="Out-of-sample predictive performance by number of iterations and shrinkage. Smaller values of the shrinkage parameter offer improved predictive performance, but with decreasing marginal improvement.", fig.alt="Out-of-sample predictive performance by number of iterations and shrinkage."}
...
```{r oobperf, echo=FALSE, out.width="100%", fig.cap="Out-of-sample predictive performance of OOB, test set, and CV methods for selecting the optimal number of iterations. The vertical axis plots performance relative to the best. The boxplots indicate relative performance across thirteen real datasets from the UCI repository. See `demo(OOB-reps)`.", fig.alt="Comparison of methods for estimating the optimal number of boosting iterations."}
knitr::include_graphics(if (knitr::is_latex_output()) {
  "oobperf2.pdf"
} else {
  "oobperf2.png"
...
methods perform similarly. However, 5-fold cross validation is
consistently the best of them. OOB, using a 33\% test set, and using a
20\% test set all have datasets for which they perform considerably worse
...
## Laplace
  ...
  Terminal node estimates   $\mbox{median}_w(y-f(\mathbf{x}))$
  ------------------------- ----------------------------------------------------
...
Contributed by Brian Kriegler [@Kriegler:2010].
  ...
  ------------------------- ----------------------------------------------------------------------------------
  Deviance                  $\displaystyle \frac{1}{\sum w_i} \left[\alpha \sum_{y_i>f(\mathbf{x}_i)} w_i(y_i-f(\mathbf{x}_i)) + (1-\alpha)\sum_{y_i\leq f(\mathbf{x}_i)} w_i(f(\mathbf{x}_i)-y_i)\right]$
  Initial value             $\mathrm{quantile}^{(\alpha)}_w(y)$
  Gradient                  $z_i=\alpha I(y_i>f(\mathbf{x}_i))-(1-\alpha)I(y_i\leq f(\mathbf{x}_i))$
  Terminal node estimates   $\mathrm{quantile}^{(\alpha)}_w(y-f(\mathbf{x}))$
  ------------------------- ----------------------------------------------------------------------------------

Notes:

- $\mathrm{quantile}^{(\alpha)}_w(y)$ denotes the weighted empirical
  quantile: after sorting $y$, it is the first value whose cumulative
  weight exceeds $\alpha\sum_i w_i$, with $\alpha=1$ returning the
  maximum.

## t-distribution

The `tdist` distribution has a degrees-of-freedom parameter \(\nu\),
which defaults to 4.

  ------------------------- --------------------------------------------------------------------
  Deviance                  $\displaystyle \frac{1}{\sum w_i}\sum_i w_i \log(\nu + (y_i-f(\mathbf{x}_i))^2)$
  Initial value             location M-estimate of $y$
  Gradient                  $\displaystyle z_i=\frac{2(y_i-f(\mathbf{x}_i))}{\nu + (y_i-f(\mathbf{x}_i))^2}$
  Terminal node estimates   location M-estimate of $y-f(\mathbf{x})$
  ------------------------- --------------------------------------------------------------------

Notes:

- The location M-estimate starts from the weighted median. It estimates
  scale as \(1.4826\) times the weighted median absolute deviation and
  iterates with weights proportional to
  \(\psi(|u|/s)/(|u|/s)\), where \(\psi(v)=v/(\nu+v^2)\).

## Huberized hinge loss

Let \(\eta_i=o_i+f(\mathbf{x}_i)\), \(q_i=2y_i-1\), and
\(u_i=q_i\eta_i\).

  ------------------------- -----------------------------------------------------------------------------------------------
  Deviance                  $\displaystyle \frac{1}{\sum w_i}\sum_i w_i L(u_i)$
  Initial value             $\displaystyle \frac{\sum w_i y_i}{\sum w_i(1-y_i)}$
  Working response          see below
  Terminal node estimates   see below
  ------------------------- -----------------------------------------------------------------------------------------------
...
Equivalently, the working response is
\[
z_i =
\begin{cases}
-4q_i, & u_i < -1,\\
-2q_i(1-u_i), & -1 \leq u_i \leq 1,\\
0, & u_i > 1.
\end{cases}
\]
For a terminal node \(S_k\), the terminal node prediction is
\[
\rho_k =
\frac{\sum_{\mathbf{x}_i\in S_k} w_i a_i}
{\sum_{\mathbf{x}_i\in S_k} w_i b_i},
\]
where
\[
a_i =
...
- This distribution requires \(y_i\in\{0,1\}\).

## Multinomial

Let \(y_{ik}\) be the indicator that observation \(i\) belongs to class
\(k\), and let
\[
p_{ik}=\frac{\exp(f_k(\mathbf{x}_i))}
{\sum_{\ell=1}^C \exp(f_\ell(\mathbf{x}_i))}.
\]

  ------------------------- -----------------------------------------------------------------------------------------------
  Deviance                  $\displaystyle -\frac{1}{\sum_i w_i}\sum_i\sum_{k=1}^C w_i y_{ik}\log p_{ik}$
  Initial value             0
  Gradient                  $z_{ik}=y_{ik}-p_{ik}$
  Terminal node estimates   $\displaystyle \frac{\sum_i w_i(y_{ik}-p_{ik})}{\sum_i w_i |y_{ik}-p_{ik}|(1-|y_{ik}-p_{ik}|)}$
  ------------------------- -----------------------------------------------------------------------------------------------

The gbm engine fits one tree per class per boosting iteration. The
multinomial deviance is computed after the last class tree in each
iteration.

## Cox Proportional Hazards

  ------------------------- -----------------------------------------------------------------
  Deviance                  $\displaystyle -\frac{2}{\sum_i w_i\delta_i}\sum_i w_i\delta_i(f(\mathbf{x}_i)-\log R_i)$
  Gradient                  $\displaystyle z_i=\delta_i - e^{f(\mathbf{x}_i)}\sum_j \frac{w_j\delta_j I(t_i\geq t_j)}{R_j}$
  Initial value             0
...
2.  Given the current \(\pmb{\rho}\), let
    \[
    p_i^{(k)}(\pmb{\rho}) =
    \frac{\sum_j I(k(j)=k)I(t_j\geq t_i)w_j
      e^{f(\mathbf{x}_j)+\rho_{k(j)}}}
    {\sum_j I(t_j\geq t_i)w_j e^{f(\mathbf{x}_j)+\rho_{k(j)}}}.
    \]

3.  Let
    $g_k=\sum_i w_i\delta_i\left(I(k(i)=k)-p_i^{(k)}(\pmb{\rho})\right)$

4.  For identifiability, fix \(\rho_K=0\). Let \(\mathbf{H}\) be the
    \((K-1)\times(K-1)\) Hessian matrix with

    1.  Set diagonal elements
        $H_{mm}=-\sum_i w_i\delta_i p_i^{(m)}(\pmb{\rho})\left(1-p_i^{(m)}(\pmb{\rho})\right)$

    2.  Set off diagonal elements
        $H_{mn}=\sum_i w_i\delta_i p_i^{(m)}(\pmb{\rho})p_i^{(n)}(\pmb{\rho})$

5.  Newton-Raphson update
    $\pmb{\rho} \leftarrow \pmb{\rho} - \mathbf{H}^{-1}\mathbf{g}$
...
- $R_i$ denotes the hazard for the risk set,
  $R_i=\sum_{j=1}^N w_jI(t_j\geq t_i)e^{f(\mathbf{x}_j)}$
...
4.  Normalized discounted cumulative gain:
    $$\frac{\sum_{1\leq i\leq n} y_i/\log_2(i+1)}{\sum_{1\leq i\leq n}
      y'_i/\log_2(i+1)},$$ 
    where $y'_1, \dots, y'_n$ is a reordering
    of $y_1, \dots,y_n$ with $y'_1 \geq y'_2 \geq \dots \geq y'_n$.
...
where $v_i=w(\operatorname{group}(i))/\left\lVert\{(j,k)\in\operatorname{group}(i)\}\right\rVert.$
